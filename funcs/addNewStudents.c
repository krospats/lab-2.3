...
char* noSpacesAllowed(char buffer[128]) {
	int isSpace = 0;
	do {
		isSpace = 0;
		...
		for (int i = 0; i < strlen(buffer) + 1; i++) {
			if (buffer[i] == ' ' && isSpace == 0) {
				printf("\nuse symbols like ( _ , - ) instead of ' '");
				isSpace = 1;
				printf("\ntry again - ");
				continue;
			}
...
char* getSubject(char buffer[128]) {
	...
	printf("\nenter subject:");
	buffer = fgets(buffer, 128, stdin);
	buffer = changeSpace(buffer);
	buffer[strlen(buffer) - 1] = '\0';
	arrayOfChars = mallocWithoutNull(strlen(buffer) + 1);
	strcpy(arrayOfChars, buffer);
	return arrayOfChars;
}
char* changeSpace(char buffer[128]) {
